...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // -------------------------------------------------------------
    // Shift Legend Toggles
    // -------------------------------------------------------------
    function initShiftToggles() {
        const selectAllBtn = document.getElementById('btn-select-all-shifts');
        const deselectAllBtn = document.getElementById('btn-deselect-all-shifts');
        const checkboxes = document.querySelectorAll('.shift-toggle-checkbox');

        checkboxes.forEach(cb => {
            cb.addEventListener('change', () => {
                const label = cb.closest('.shift-toggle-item');
                if (label) {
                    if (cb.checked) label.classList.remove('shift-disabled');
                    else label.classList.add('shift-disabled');
                }
            });
        });
        ...
        if (selectAllBtn) {
            selectAllBtn.addEventListener('click', () => {
                checkboxes.forEach(cb => {
                    cb.checked = true;
                    const label = cb.closest('.shift-toggle-item');
                    if (label) label.classList.remove('shift-disabled');
                });
            });
        }
        ...
        if (deselectAllBtn) {
            deselectAllBtn.addEventListener('click', () => {
                checkboxes.forEach(cb => {
                    cb.checked = false;
                    const label = cb.closest('.shift-toggle-item');
                    if (label) label.classList.add('shift-disabled');
                });
            });
        }
    }
    ...
    function triggerExport(type) {
        ...
        legend.innerHTML = `
            <div style="margin-top: 30px; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px; font-family: 'Inter', sans-serif; display: flex; gap: 20px; flex-wrap: wrap; justify-content: center; background: #f8fafc; color: #334155; font-size: 0.85rem;">
                <div style="display:flex; align-items:center; gap:8px;"><span style="width:18px;height:18px;background:#FF9500;border:1px solid #e08300;border-radius:4px;"></span> <b>F:</b> Full Day (8:30am - 10:00pm)</div>
                <div style="display:flex; align-items:center; gap:8px;"><span style="width:18px;height:18px;background:#5AC8FA;border:1px solid #48addb;border-radius:4px;"></span> <b>M:</b> Morning (8:30am - 5:30pm)</div>
                <div style="display:flex; align-items:center; gap:8px;"><span style="width:18px;height:18px;background:#FF3B30;border:1px solid #de332a;border-radius:4px;"></span> <b>N:</b> Night (1:00pm - 10:00pm)</div>
                <div style="display:flex; align-items:center; gap:8px;"><span style="width:18px;height:18px;background:#30B0C7;border:1px solid #2897aa;border-radius:4px;"></span> <b>Mw:</b> Morning Wknd (8:30am - 8:30pm)</div>
                <div style="display:flex; align-items:center; gap:8px;"><span style="width:18px;height:18px;background:#FF2D55;border:1px solid #de2548;border-radius:4px;"></span> <b>Nw:</b> Night Wknd (11:00am - 10:00pm)</div>
                <div style="display:flex; align-items:center; gap:8px;"><span style="width:18px;height:18px;background:#ffffff;border:1px solid #cbd5e1;border-radius:4px;"></span> <b>No:</b> Cashier (8:30am - 7:30pm)</div>
                <div style="display:flex; align-items:center; gap:8px;"><span style="width:18px;height:18px;background:#A2845E;border:1px solid #8d7250;border-radius:4px;"></span> <b>Nh:</b> Cashier Night (10:30am - 9:30pm)</div>
                <div style="display:flex; align-items:center; gap:8px;"><span style="width:18px;height:18px;background:#34C759;border:1px solid #2db04d;border-radius:4px;"></span> <b>Off:</b> Off Day</div>
            </div>
        `;
    ...
    function initRosterBoard() {
        ...
        btnGen.addEventListener('click', () => {
            ...
            const enabledShifts = Array.from(document.querySelectorAll('.shift-toggle-checkbox:checked')).map(cb => cb.value);
            if (enabledShifts.length === 0) {
                showToast('Please select at least one shift type from the Shift Legend!', 'error');
            ...
            fd.append('year', state.year);
            fd.append('month', state.month);
            fd.append('enabled_shifts', JSON.stringify(enabledShifts));
