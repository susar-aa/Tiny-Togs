...
try {
    ...
    $enabled_shifts = ['F', 'M', 'N', 'Mw', 'Nw', 'No', 'Nh', 'Off'];
    if (isset($_POST['enabled_shifts']) && !empty($_POST['enabled_shifts'])) {
        $parsed_shifts = json_decode($_POST['enabled_shifts'], true);
        if (is_array($parsed_shifts) && count($parsed_shifts) > 0) {
            $enabled_shifts = $parsed_shifts;
        }
    }
    ...
    if ((int)$stmt_count->fetchColumn() === 0) {
        ...
        $induwara_token = getEnvVar('INDUWARA_API_KEY', 'ilk_live_10ede3621d457ccba9d0b0fdfec027ef48bf41cfd263d250');
        $ssl_verify = (bool)getEnvVar('CURL_SSL_VERIFY', false);
        curl_setopt($ch, CURLOPT_HTTPHEADER, ["Authorization: Bearer {$induwara_token}"]);
...
function solveRotatingShifts($idx, $working_rotating, $day_idx, $calendar_days, &$roster, $employees, &$counts, &$off_counts, &$valid_choices, $req, &$found_count, $relaxation_level, $core_working_count = 10) {
    global $timeout_reached, $enabled_shifts;
    ...
    $opts = array_values(array_intersect($opts, $enabled_shifts));
    if (empty($opts)) {
        $opts = array_values(array_intersect(['F', 'M', 'Mw', 'N', 'Nw', 'No', 'Nh', 'Off'], $enabled_shifts));
        if (empty($opts)) $opts = ['F'];
    }
    ...
    if (empty($final_opts) && $relaxation_level >= 3) {
        $final_opts = array_values(array_intersect(['F', $morning_code, $night_code], $enabled_shifts));
        if (empty($final_opts)) {
            $final_opts = array_values(array_intersect(['F', 'M', 'Mw', 'N', 'Nw', 'No', 'Nh', 'Off'], $enabled_shifts));
            if (empty($final_opts)) $final_opts = ['F'];
        }
...
function solveRosterJoint($day_idx, $calendar_days, $employees, &$roster, &$off_counts, $approved_leaves, $male_anchor_ids, $relaxation_level) {
    global $max_day_reached, $iteration_count, $timeout_reached, $deepest_fail_reason, $enabled_shifts;
    ...
    if (!in_array('Off', $enabled_shifts)) {
        $valid_off_groups = [[]];
    }
    ...
    foreach ($valid_off_groups as $off_group) {
        ...
        foreach ($employees as $emp) {
            ...
            if ($role === 'Anchor') {
                if (in_array('F', $enabled_shifts)) {
                    $temp_roster[$emp_id] = 'F';
                } else {
                    $enabled_work = array_values(array_intersect(['F', 'M', 'Mw', 'N', 'Nw', 'No', 'Nh', 'Off'], $enabled_shifts));
                    $temp_roster[$emp_id] = !empty($enabled_work) ? $enabled_work[0] : 'F';
                }
            } elseif ($role === 'Cashier') {
                ...
                if ($is_valid_nh_day) {
                    ...
                    if ($nh_count < 5 && in_array('Nh', $enabled_shifts)) {
                        $temp_roster[$emp_id] = 'Nh';
                    } elseif (in_array('No', $enabled_shifts)) {
                        $temp_roster[$emp_id] = 'No';
                    } else {
                        $enabled_work = array_values(array_intersect(['No', 'Nh', 'F', 'M', 'Mw', 'N', 'Nw', 'Off'], $enabled_shifts));
                        $temp_roster[$emp_id] = !empty($enabled_work) ? $enabled_work[0] : 'No';
                    }
                } else {
                    if (in_array('No', $enabled_shifts)) {
                        $temp_roster[$emp_id] = 'No';
                    } else {
                        $enabled_work = array_values(array_intersect(['No', 'Nh', 'F', 'M', 'Mw', 'N', 'Nw', 'Off'], $enabled_shifts));
                        $temp_roster[$emp_id] = !empty($enabled_work) ? $enabled_work[0] : 'No';
                ...
                $day_of_week = (int)date('N', strtotime($date));
                $target_sc = in_array($day_of_week, [1, 3, 5]) ? ($is_weekend_or_holiday ? 'Mw' : 'M') : 'F';
                if (!in_array($target_sc, $enabled_shifts)) {
                    $enabled_work = array_values(array_intersect(['F', 'M', 'Mw', 'N', 'Nw', 'No', 'Nh', 'Off'], $enabled_shifts));
                    $target_sc = !empty($enabled_work) ? $enabled_work[0] : 'F';
                }
                $temp_roster[$emp_id] = $target_sc;
            } else {
        ...
        if (!in_array('M', $enabled_shifts) && !in_array('Mw', $enabled_shifts)) {
            $req['m_males'] = 0; $req['m_females'] = 0; $req['m_good'] = 0; $req['m_total'] = 0;
        }
        if (!in_array('N', $enabled_shifts) && !in_array('Nw', $enabled_shifts)) {
            $req['n_males'] = 0; $req['n_females'] = 0; $req['n_good'] = 0; $req['n_total'] = 0;
        }
